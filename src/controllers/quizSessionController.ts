...
export const submitAnswer = async (req: Request, res: Response) => {
  try {
    ...
    const score = session.answers.reduce((sum, a) => {if (a.isCorrect) { return sum + a.points};
  return sum }, 0);
    const maxScore = session.answers.reduce((sum, a) => sum + a.points, 0);
    const durationInSeconds = (new Date().getTime() - new Date(session.startedAt).getTime()) / 1000;
